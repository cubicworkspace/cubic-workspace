				...
				<div class="container">
					...
					<div class="grid-item" data-colspan="5" data-rowspan="6">
							<a href="{{ url('/website/partnership') }}/{{ $row->id }}/{{ str_slug($row->name) }}" class="top-destination-image-bg" style="background-image:url('{{ asset('upload/companypartnership') }}/{{ $row->favicon }}');">
								<div class="relative">
									<h4>{{ $row->name }}</h4>
									<span>{{ !empty($row->city->name) ? $row->city->name : '-' }}  - {{ !empty($row->country->name) ? $row->country->name : '-' }} </span>
								</div>
							</a>
						</div>
						@endforeach
			...
			<div class="modal-content">
				...
				<div id="modal-login-form-wrapper">
					...
					<!-- Begin # Login Form -->
			            ...
			            <form action="{{ route('website.partnershipregister') }}" method="POST" enctype="multipart/form-data">
						{{csrf_field()}}
						<input type="hidden" name="codecompanypartnership" value="{{ $no }}">
						<div class="modal-body pb-5">					
							...
							<div class="form-group"> 
	   								 {!! Form::select('codecountry', $country, null,
	   								  ['class' => 'form-control', 'id' => 'id','required' => 'required', 'value' => 'name',
							...
							<div class="form-group"> 
									<input type="file" class="form-control" id="image" name="image" placeholder="Logo" required>
							</div>			
						</div>						
